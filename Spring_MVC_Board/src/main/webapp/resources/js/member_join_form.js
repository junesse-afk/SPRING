$(function(){
	...
	$('#passwd2').on('change', checkPass2); // passwd 같은지 확인 이벤트
	...
	- 웹페이지(브라우저)의 갱신(refresh) 없이도 서버로 요청을 수행하고
	...
	<기본 문법>
	$.ajax({
		type: xxx,	//ajax로 요청시 사용할 메서드 =요청방식(Get or Post)
	...
	*/
	...
	function checkPass() {
		...
		if (lengthRegex.exec(passwd)) {
			let count = 0; //자리수 확인하기 위해 카운트 변수 선언
			let engUpperRegex = /[A-Z]/; //대문자 판별
	...
	function checkPass2(){
		...
		if (passwd == passwd2){
			// '비밀번호 일치', blue
			color="blue";
			$('#checkPasswd2Result').text('비밀번호 일치').css('color', color);
			checkPasswd2 = true;
		} else {
			// '비밀번호 불일치', red
			color="red";
			$('#checkPasswd2Result').text('비밀번호 불일치').css('color', color);
			checkPasswd2 = false;
		}
