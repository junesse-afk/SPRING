...
public class Test2Controller_Push {
		...
		// => 컨트롤러 클래스 메서드 JSP파일이 아니므로 JSP 내장 객체 (request)가 존재하지 않고
		// 컨트롤러 클래스 메서드 파라미터에 HttpServletRequest 타입 변수가 명시되어 있지도 않음
	
		// => JSP에서는 톰캣에 의해 doGet() 또는 doPost() 메서드 호출 시
		// HttpServeltRequest 객체가 자동으로 파라미터로 전달되어졌다.
		// 스프링에서는 의존 주입(DI = Dependency Injection)을 통해 객체를 외부로부터 주입 받을 수 있음
		// => push() 메서드 선언부의 파라미터 타입으로 HttpServletRequest 타입변수를 선언하기만 하면
		// 스프링에 의해 해당 메서드 호출 시점에 HttpServletRequest 객체가 자동으로 주입됨
	
		// -----------------------------------------------------------
		
		// 2. 스프링 전용 Model 객체의 addAtrribute() 사용 => 스프링에서 사용하는 방법
		// =>org. springframework.ui.Model 타입을 파라미터로 지정 시
		// 데이터 저장이 가능한 Model 객체를 자동으로  주입 받을 수 있음
		// =>httpservletRequest 객체와 데이터 저장 측면에서 유사하며
		// java.util.Map 객체 기반으로 만든 스프링에서 제공하는 데이터 공유 전용 객체
		// (스프링 전용 라이브러리 필요)
		// =>Mode 객체에서 데이터 저장시 스프링에 의해 자동으로 httpservletRequest 객체에 저장
		// => request.setAtrribute()와 마찬가지로 model.addAtrribute() 메서드
		// => request 객체의 범위(Scope) 동일 (클라이언트 요청에 대한 서버 응답 지점까지)
		// => request 객체와 데이터 저장용도로 동시 사용 불가
		//    (일반적인 데이터 저장 시 HttpServletRequest 객체 대신 Model 객체 사용)
		
	
	
		// 1번 방법) 매핑 메서드 파라미터를 통해 필요한 HttpServletRequest 객체 주입 받기
...
//			// request, response 객체를 전달 받는 것과 형태가 동일함
//			//-------------------------------------------------
//			// request 객체의 setAtribute() 메서드 활용하여
...
//		}
		...
		// 2번 방법) 매핑 메서드 파라미터를 통해 Model 타입 객체를 자동으로 주입받아 자동으로 저장
		@GetMapping("push")
