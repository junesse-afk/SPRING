...
public class HomeController {
	 ...
	 * - 기본 문법 : @RequestMapping(value = "/", method = RequestMethod.xxx)
	 *            public 리턴타입 메서드명([파라미터...]){}
	 *   => value 속성은 매핑에 사용될 매핑 URL을 문자열로 지정하고
	 *      method 속성은 RequestMethod라는 enum 타입의 값으로 GET, POST, PUT, DELETE 
	 *   => 어노테이션 아래에 선언된 메서드는 해당 매핑 주소가 요청되면 자동으로 호출됨.
	 *      이때, 메서드명은 무관하며, 스프링에 의해 자동으로 호출되기 위한 메서드 형태
	 *      또한, 메서드 리턴타입은 별다른 지시 사항이 없을 경우 String 타입으로 지정
	...
	public String BoardWrite() {
		...
		return "redirect:/BoardListt";
		//redirect 뒤에는 url를 사용
	}
	
	@RequestMapping(value="/BoardListt", method = RequestMethod.GET)
